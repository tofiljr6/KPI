...
const DATAMODEL_SYSTEM = [
  ...
  'transparent tables. Pick the ONE primary table, then lay out 1-4 steps.',
  '',
  'One step = one single-table SELECT. The runtime (SAP QuerySet) CANNOT do JOINs or',
  'sub-selects. So whenever the answer needs columns from two or more tables, you MUST',
  'split it into one step per table — never write a JOIN:',
  '- step 1 selects the linking key from table A (e.g. ADDRNUMBER from BUT020),',
  '- step 2 filters table B on that key: keyField = the column name, dependsOn = that',
  '  column (e.g. ADRC where ADDRNUMBER = <value from step 1>).',
  'The runtime feeds step 1\'s result into step 2 automatically. Do not pad: if one table',
  'answers the whole request, one step is fine.',
  '',
...
const SKILL_SYSTEM = [
  ...
  '- queries: one entry per step of the plan, in execution order — one single-table SELECT',
  '  each, NEVER a JOIN. Each has a short human name, a one-or-two sentence description',
  '  (including which placeholder it needs and, if it depends on an earlier step, where that',
  '  value comes from), the table, the fields, and a WHERE clause using {placeholder} tokens,',
  '  e.g. "PARTNER = \'{partner}\'". Placeholder names are lowercase and match the field they',
  '  filter. A step that consumes an earlier step\'s output filters on a {placeholder} named',
  '  exactly after that column, e.g. step 2: "ADDRNUMBER = \'{addrnumber}\'" where {addrnumber}',
  '  is a field step 1 selected. Never invent field names.',
